...
@Component({
    templateUrl: 'air-cargo-status.component.html'
})
export class AirCargoStatusReportComponent implements OnInit {
    ...
    onSearch() {
        ...
        this._dataService.getDownload("Export/Generate_Air_Cargo_Status", _jsonGet, { responseType: 'arraybuffer' })
            .subscribe((data: any) => {
                this._dataService.commonDownloadExcelFile(data, "RPT_CARGO_STATUS" + this._dataService.timeFormatAMPM() + ".xls");
                this._loaderService.display(false);
            });
    }
}