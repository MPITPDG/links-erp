...
@Component({
    templateUrl: 'tracking-air-shipment.component.html'
})
export class TrackingAirShipmentReportComponent implements OnInit {
    ConsigneeFormatList: Array<any> = [];
    ConsigneeList: Array<any> = [];
    SourcingAgentList: Array<any> = [];
    CityList: Array<any> = [];
    Type: string = 'ConsigneeFormat';
    ConsigneeFormat: any = null;
    Consignee: any = null;
    ...
    pageLoad() {
        ...
        this._dataService.getData("Export/Fill_dsr_PAGELOAD", {})
            .subscribe((data: any) => {
                ...
                this.ConsigneeFormatList = data.Table2;
    ...
    onSearch() {
        ...
        this._dataService.getDownload("Export/Generate_Tracking_Air_Shipment_Report", _jsonGet, { responseType: 'arraybuffer' })
            .subscribe((data: any) => {
                this._dataService.commonDownloadExcelFile(data, "RPT_TRACKING_AIR_SHIPMENTS_REPORT_" + this._dataService.timeFormatAMPM() + ".xls");
                this._loaderService.display(false);
            });
    }
}