...
@Component({
    ...
    templateUrl: 'exp_sp_factoryStuff_ContainerIU_CLP_New.html'
})
export class ExptSpContainerIUFSNEW implements OnInit {
    Add:string="Add";
    vslrotation: any = "";
    vslport: any = "";
    vsldet: any = "";
    ...
    onfillAgent(vsldt)
    {
        if (vsldt !="")
        {
          this.vessellist = this.vessel.filter((code: any) => code.VALUEFILED=== vsldt);
    ...
    OnChangeExjob(exjobno)    
    {
        ...
        {
        this.loaderService.display(true);  
        const jsonExport = { exptno: exjobno, userid: this._loginService.getLogin()[0].CMPID,containerno:this.THIS_CONTAINER_NO };
    ...
    onchangecontainer(contno){
            this.loaderService.display(true);  
        const jsonExport = { vslno:this.Vsl.split("|")[0], vslagent:this.Vslagt, 
            ...
            this._dataService.getData('Export/EXP_SHIPMENT_PROGRESS_CONTAINERFS_GETDATA', jsonExport)
                .subscribe((data: any) => {
                    if (data.Table[0].STATUS == 100) {
                       ...
                       this.cntsize=data.Table2[0].CONTAINERSIZE;
                       this.cntsize1=data.Table2[0].CONTAINERSIZE1;
                       this.contstatus=data.Table2[0].CONTAINERSTATUS;
                       this.Movetype=data.Table2[0].MOVETYPE;
                       this.destport=data.Table2[0].DESTPORT;
                       this.finalsubmitmode='UPDATE'
                      this.loaderService.display(false);  
                    }
    ...
    childreset()
 {
    ...
    finalSubmit()
    {
        ...
        .subscribe((data: any) => {
            ...
            if (data.Table[0].STATUS == "100") {
                this._toasterService.toast('info', 'Success', data.Table[0].STATUSTXT);    
                this.masterReset();     
                        
                this.loaderService.display(false);                        
            }
            else {
                this._toasterService.toast('error', 'Error', data.Table[0].STATUSTXT);
                this.loaderService.display(false);
            }
        }); 
    }
