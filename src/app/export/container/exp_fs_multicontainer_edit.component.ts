...
@Component({
    ...
    templateUrl: 'exp_fs_multicontainer_edit.component.html'
})
export class FSmulticontainerEdit implements OnInit {
    ...
    noofcontainers;
    public disable_grnerate_btn :boolean=true;
    public disable_noofcontainer :boolean=true;
    public disable_destport :boolean=true;
    public disable_contstatus :boolean=true;
    public disable_contsize :boolean=true;
    public disable_contsize1 :boolean=true;
    public disable_movetype :boolean=true;
    public disable_exptno :boolean=false;
    lastUpdatedRowId: number | null = null;
selectedRowId: number | null = null;
    ...
    OnChangeExjob(exjobno)    
    {
        ...
        this._dataService.getData('Export/ContainerFS_FillExptDtls', jsonExport)
            ...
            //     }
            //     else{
               this.TOTAL_NOOFPKGS=data.Table[0].EXPT_NOOFPKGS;
               this.TOTAL_CBM=data.Table[0].EXPT_VOLUME;
               this.TOTAL_NO_OF_PCS=data.Table[0].EXPT_NOOFPCS;
               this.TOTAL_NO_GR_WT=data.Table[0].EXPT_GRWT;
               this.TOTAL_NO_NET_WT=data.Table[0].EXPT_NETWT;
               this.disable_exptno=true;  
    ...
    AddChild()
    {
        ...
        .subscribe((data: any) => {
            ...
            if (data.Table[0].STATUS == "100") {
                ...
                const row = this.containerClp.find(r => r.ID === this.id);
                        if (!row) return;
    ...
    getLatestEntryID(dataTable: any[]): number | null {
          if (dataTable.length === 0) {
            return null;
          }
          // Assuming ID is a number and higher IDs mean more recent entries
          const highestID = Math.max(...dataTable.map(item => item.ID));
          return highestID;
        }
    EditchildExptNo(Id)
    {   this.Add="Update";
        ...
        this._dataService.getData("Export/ContainerFS_DELETE", jsondelete)
            .subscribe((data: any) => {
                if (data.Table[0].STATUS == "100") {
                    this._toasterService.toast('info', 'Success', data.Table[0].STATUSTEXT);
                    this.containerClp=data.Table;
                   this.AVAILABLE_NOOFPKGS=data.Table1[0].AVAI_PKGS
                    this.AVAILABLE_NO_OF_PCS=data.Table1[0].AVAI_PCS
                    this.AVAILABLE_NO_GR_WT=data.Table1[0].AVAI_GRWT
                    this.AVAILABLE_NO_NET_WT=data.Table1[0].AVAI_NETWT
                    this.loaderService.display(false);   
                }
            });
        }
    ...
    finalSubmit()
    {
        ...
        if (this.Vslshpgline == undefined || this.Vslshpgline == "") {
            ...
            return false;
                ...
                const invalid = this.containerClp.some(obj => 
            !obj.CONTAINERNO || obj.CONTAINERNO.startsWith("CONTAINERNO")
        );
