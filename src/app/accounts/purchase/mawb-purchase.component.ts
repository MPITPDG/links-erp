...
@Component({
    ...
    templateUrl: './mawb-purchase.component.html',
    styles: [`
    a:hover,a:focus{
  ...
  export class MAWBPurchaseComponent implements OnInit {
    ...
    Deduction_step1:string=""; Tottaxableamt:any;seltaxableamt: any = 0;
    ...
    JobNo_step2: string = ""; Amount_step2: string = ""; actpassedamount_step2: string = "0"; 
    ACCBANKDTLSID: string = "0"; Netdeduction_step2: string = "";
    NetAmt_step2: string = ""
    ...
    public disableamt:boolean=false;
    public disablededuction:boolean=false;
      ...
      this.Clearall_MAWBTMP();
       ...
       this._dataService.getData("Accounts/ACC_PURCHASE_PAGELOAD_MAWB", jsonparam)
        ...
        this._dataService.getData("Accounts/ACC_PURCHASE_PAGELOAD", jsonparam1)
          .subscribe((data: any) => {
           this.AccountNameStep2List = data.Table;
           });
        }
        Clearall_MAWBTMP() {
        ...
        verifyPermission(formId, userMode) {
          ...
          this.fromright_dt = data.Table[0].blockuntildt;
  ...
  getCopyOfOptions(): IMyDpOptions {
    return JSON.parse(JSON.stringify(this.myDatePickerOptionsInv));
        ...
        PurchaseDate_Step2Changed(ev){
          this.purchasedate_step2=ev.formatted
        console.log(ev)
        }
        PurchaseDate_Step1Changed(ev){
        ...
        GetActName_Step2(Acctcode) {
          this.Item_step2=undefined;
          this.ACCTNAME = this.AccountNameStep2List.filter((code: any) => code.ACCTCODENAME === Acctcode)[0].ACCTNAME;
          this.Itemslist = this.Allitemlist.filter((code: any) => code.ACCTCODE === Acctcode.split("|")[0]);
          //this.EnableDisableamtdeduction(Acctcode);
        }
        EnableDisableamtdeduction(Acctcode)
        {
        ...
        Addchildrcrdstep1() {
          ...
          if(this.ID=="" || this.ID==undefined || this.ID=="0"){
            let filterchldactcode = this.Accountlist_step1.filter((filter: any) => filter.CLIENT == this.AccountName_step1.split("|")[0]);
          ...
          const jsonparam = {
            ...
            MAWBNO:this.mawbno ,//this.Item_step1,
      ...
      deletechildrecdstep1(id) {
            if (confirm('Are you sure want to delete the record ?')) {
            this.loaderService.display(true);
            const jsondelete = { ID: id, VGUID: this._loginService.getLogin()[0].GUID }
      ...
      Add_Mainstep1() {
        ...
        let filteractcode = this.Accountlist_step1.filter((filter: any) => filter.CLIENT == "G100235");
        ...
        let frtrebateactcode = this.Accountlist_step1.filter((filter: any) => filter.CLIENT == "G100186");
        let afcommactcode = this.Accountlist_step1.filter((filter: any) => filter.CLIENT == "G100153");
       ...
       // console.log(this.STRTXT);
         ...
         this._dataService.getData("Accounts/ACC_PURCHASE_MAWBNO_PAGELOAD_NG", jsonparam)
          ...
          this.Tottaxableamt=data.Table1[0].TOTTAXAMT  
          this.buttonStatus =true; 
          
          if(this.chkIsTDS_step2==true){
            this.disabletdsbtn=false;
             }
          else
          {
