...
@Component({
    ...
    templateUrl: './bank-reconcil-rpt.component.html',
    styles: []
  })
  export class BankReconcilStatementComponent implements OnInit {
        ...
        this._loaderService.display(false);
            ...
            FillAccount(){
              ...
              ToDateChanged(ev){
               ...
               {
               // this.OutstandingData();
                this.tbl1.forEach((row: any) => {
                  this.dataForExcel.push(Object.values(row))
                  });
                  this.tbl2.forEach((row: any) => {
                    this.recdataForExcel.push(Object.values(row))
                    });
                    this.tbl10.forEach((row: any) => {
                      this.tbl10dataForExcel.push(Object.values(row))
                      });
                      this.tbl11.forEach((row: any) => {
                        this.tbl11dataForExcel.push(Object.values(row))
                        });
                        this.tbl12.forEach((row: any) => {
                          this.tbl12dataForExcel.push(Object.values(row))
                          });
                 let openingbalance=(this.rptopbal+this.rptDrBalance)  -this.rptCrBalance
                 let closingbalance=(openingbalance+this.rptCurrentDrBal)-this.rptCurrentCrbal
                let reportData = {
                   data: this.dataForExcel,
                   recdata:this.recdataForExcel,
                   headers: ['Entry No','Entry Date','Status','Cheque No','Cheque Date','Clearing Date','Amount'],
                   paytotal:this.exIssueTotal,//['','','','','',this.IssueTotal],
                   rectotal:this.exRecTotal,//['','','','','',this.RecTotal],
