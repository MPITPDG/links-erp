...
@Component({
    templateUrl: 'entry-air.component.html',
    styleUrls: ['../../../../node_modules/@ng-select/ng-select/themes/default.theme.css'], 
    encapsulation: ViewEncapsulation.None
})
export class EntryAirImportComponent implements OnInit {
    ...
    ArrivalPortList: Array<any> = [];
    TruckOurList: Array<any> = [];
    AirlineNameList: Array<any> = [];
    TranshipmentAirlineList: Array<any> = [];
    ...
    TranshipmentAirline: any = null;
    TranshipmentFlightNo: any = '';
    TranshipmentFlightDate: any = null;
    ...
    ngOnInit(): void {
        this._route.params.subscribe(params => {
            if (params["import-id"]) {
                ...
                this.Iu_Mode = 'UPDATE';
                this.FormHeader = 'UPDATE - IMPORT AIR';
                this.verifyPermission(125, "Modify");
            }
            else {
                this.FormHeader = 'ADD - IMPORT AIR';
                this.Iu_Mode = 'ADD';
                this.verifyPermission(125, "Add");
            }
        });
    }
    ...
    fillCommonDropdownList() {
        ...
        this._dataService.getData("Import/Fill_Dropdown", _jsonData)
            .subscribe((data: any) => {
                ...
                this.AirlineNameList = data.Table1;
                this.TranshipmentAirlineList = data.Table1;
                ...
                this.ArrivalPortList = data.Table3;
                this.TruckOurList = data.Table5;
                this.SupplierList = data.Table6;
                this.IncotermList = data.Table7;
                this.SteTypeChargeList = data.Table8;
                this.BillOfEntryNoList = data.Table9;
                this.TypeOfPackage2List = data.Table10;
                this.BusinessAssociateList = data.Table11;
                this.userdtls=data.Table13;
    ...
    GetLowestEtaDate() {
        let Drp1 = this.AirlineName;
        let Drp2 = this.TranshipmentAirline;
        if (Drp1 != Drp2) {
    ...
    totalValidation(): boolean {
        ...
        if (!this.AirlineName) { alert("Select Airline Name..."); return false; }
        else if (!this.FlightNo) { alert("Please enter Flight No."); document.getElementsByName("FlightNo")[0].focus(); return false; }
        else if (!this.FlightDate) { alert("Please enter Flight Date."); return false; }
        else if (this.DirectVessel == 'NO') {
            if (!this.TranshipmentAirline) { alert("Please select Transhipment Airline Name."); return false; }
            else if (this.AirlineName == this.TranshipmentAirline) { alert("Transhipment Airline and Airline Name should not be Same for Direct Airline No."); return false; }
            if (!this.TranshipmentFlightNo) { alert("Please enter Transhipment Flight No."); document.getElementsByName("TranshipmentFlightNo")[0].focus(); return false; }
            if (!this.TranshipmentFlightDate) { alert("Please enter Transhipment Flight Date."); return false; }
            if (!this.TranshipmentPort) { alert("Please select Transhipment Port Name."); return false; }
        }
        if (!this.ArrivalPort) { alert("Please select Arrival Port Name."); return false; }
        if (!this.ExpectedArrDate) { alert("Please select Expected Arrival Date."); return false; }
        if (!this.MAWBNo && !this.HAWBNo) { alert("Please enter any one of following details MAWB No. Or HAWB No."); document.getElementsByName("MAWBNo")[0].focus(); return false; }
        if (this.MAWBNo != '') {
            if (this.MAWBNo.length != 11) { alert("MAWB No should be 11 Numbers."); document.getElementsByName("MAWBNo")[0].focus(); return false; }
            if (!this.MAWBDate) { alert("Please select MAWB Date."); return false; }
            else if (!this._dataService.DateFromAndToComparision(this.MAWBDate.formatted,this._dataService.getCurrentdate())) { alert("MAWB Date should be Less than or Equal to " + this._dataService.getCurrentdate()); return false; }
        } else if (this.HAWBNo != '') {
            if (!this.HAWBDate) { alert("Please select HAWB Date."); return false; }
            else if (!this._dataService.DateFromAndToComparision(this.HAWBDate.formatted,this._dataService.getCurrentdate())) { alert("HAWB Date should be Less than or Equal to " + this._dataService.getCurrentdate()); return false; }
        ...
        if (this.dtINVOICE.length == 0) { alert("Please Enter Atleast one Invoice No."); document.getElementsByName("InvoiceNo")[0].focus(); return false; }
        else if (!this.Incoterm) { alert("Please select Incoterm.."); return false; }
        else if (this.InvValue == "") { alert("Invoice Value should not be empty"); return false; }
        else if (!this.InvCurrency) { alert("Please select Currency of Invoice."); document.getElementById("InvCurrency").focus(); return false; }
        else if (this.InvExchRate == "") { alert("Invoice Exch.Rate should not be empty"); document.getElementById("InvExchRate").focus(); return false; }
        ...
        else if (this.BillOfEntryDate != null) { if (!this._dataService.DateFromAndToComparision(this.BillOfEntryDate.formatted,this._dataService.getCurrentdate())) { alert("Bill Of Entry Date should be Less than or Equal to " + this._dataService.getCurrentdate()); return false; } }
        ...
        //     }
        // }
        if (!this.InvoiceDateValidation('InvoiceDate')) { return false; }
    ...
    InsertAllDatas(mode) {
        ...
        let fk_Vsl_Air_name = "";//Sea
        let fk_Agt_name = "";//Sea
        let fk_Trans_vsl_name = "";//Sea
        let ArrivingDateAtICD = "";//Sea
        let GlDate = "";//Sea
        ...
        let fk_ArilineId = (this.AirlineName)? this.AirlineName:"";//Air
        let FightNo = this.FlightNo;//Air
        let FightDate = (this.FlightDate)? this.SqlDate(this.FlightDate.formatted) : "";//Air
        let fk_ArilineId_Transhipment = (this.TranshipmentAirline)? this.TranshipmentAirline:"";//Air
        let FightNo_Transhipment = this.TranshipmentFlightNo;//Air
        let FightDate_Transhipment = (this.TranshipmentFlightDate)? this.SqlDate(this.TranshipmentFlightDate.formatted) : "";//Air
        let fk_ArrivalPort = (this.ArrivalPort)? this.ArrivalPort:"";//Air
        let Exp_Arrival_Date = (this.ExpectedArrDate)? this.SqlDate(this.ExpectedArrDate.formatted) : "";//Air
        let Act_Arrival_Date = (this.ActualArrDate)? this.SqlDate(this.ActualArrDate.formatted) : "";//Air

        let Mbl_Mabl_NO = this.MAWBNo;
        let Mbl_Mabl_DT = (this.MAWBDate != null) ? this.SqlDate(this.MAWBDate.formatted) : "";
        let Bbl_Habl_NO = this.HAWBNo;
        let Bbl_Habl_DT = (this.HAWBDate != null) ? this.SqlDate(this.HAWBDate.formatted) : "";
        ...
        let DeliveryClerk = this.DeliveryClerk;
        ...
        let _jsonData = {
            ...
            GlDate: GlDate,
            LastFreeDate: freedt,//LastFreeDate,
            ...
            .subscribe((data: any) => { 
                ...
                if (data.Table1[0].STATUS == '100') {
                    if (this.ImportId == 0) {
                        ...
                        this._router.navigate(['/import/air/search']);
                    }
                } else {
